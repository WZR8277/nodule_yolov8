...
class ConfusionMatrix:
    ...
    def __init__(self, nc, conf=0.25, iou_thres=0.45, task='detect'):
        ...
        self.total_diou = 0  # 用于累积所有批次的 DIoU 总和
        self.total_boxes = 0  # 用于累积所有处理的检测框数量
    ...
    def process_batch(self, detections, labels):
        ...
        # # Compute DIoU between ground truth and detected boxes
        # diou = box_diou(labels[:, 1:], detections[:, :4])
        # # Accumulate DIoU and detection counts
        # self.total_diou += diou.sum().item()  # Sum the DIoU for the current batch
        # self.total_boxes += detections.shape[0]  # Count the number of detected boxes in the batch
